...
slave向master发送同步命令，主服务器执行bgsave命令，生成rdb文件，并将执行命令期间的写操作记录到缓冲区，向从服务器发送rdb和缓冲区的写操作

#### 部分复制

master维护一个复制缓冲区(固定长度的FIFO队列)，记录slave的复制偏移量，salve发送同步命令后，master比对不一致则发送continue命令通知部分复制，再发送偏移量之后的写命令