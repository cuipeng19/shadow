...
* CGLIB：对类代理，不代理final，采用FastClass机制，对一个类的方法建立索引，通过索引直接调用相应方法
...
远程过程调用，包含传输协议(HTTP、TCP)、文本协议(xml、json)


## 限流算法

* 固定窗口计数器法：规定单位时间处理的请求数量，无法保证限流速率
* 滑动窗口计数器法：把时间分片
* 漏桶算法：请求存放桶中，再从桶中拿出执行
* 令牌桶算法：请求处理前先拿令牌，处理完成后删除令牌


## 高可用

单点故障时仍能提供服务/某功能的请求成功比例

### 提高系统可用性方法

集群，限流，超时和重试，熔断，异步调用，异步调用，缓存